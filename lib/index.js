...
const getDirectories = source => readdirSync(source).filter(
    name => !(name === ".vuepress") && isDirectory(join(source, name)));
const hasReadme = source => readdirSync(source).findIndex(
                                name => name.toLowerCase() === "readme.md" &&
                                        !isDirectory(join(source, name))) > -1;
...
/**
 * Returns name to be used in menus after removing navigation prefix, prefix
 * numbers used for ordering and `.`, `-`, `_` and spaces.
 *
 * @param   {string}  path                  - File path to get name for.
 * @param   {Object}  options               - Options
 * @param   {string}  options.navPrefix     - Navigation order prefix if
 *     present.
 * @param   {boolean} options.stripNumbers  - Whether to strip numbers.
 * @returns {string}                        - Name to be used in navigation.
 * @example
 * getName("/some/path/nav-01-how", { navPrefix: "nav", stripNumbers: true });
 * // how getName("/some/path/nav.01.how", { navPrefix: "nav", stripNumbers:
 * true }); // how
 */
...
// Load all MD files in a specified directory and order by metadata 'order'
// value
function getChildren(parent_path, dir, recursive = true) {
  // CREDITS: https://github.com/benjivm (from:
  // https://github.com/vuejs/vuepress/issues/613#issuecomment-495751473)
  ...
  files =
      glob.sync(parent_path + (dir ? `/${dir}` : "") + pattern).map(path => {
        ...
        return {
          path,
          order : path === "" && order === undefined
                      ? 0
                      : order // README is first if it hasn't order
        };
      });
 ...
 *
 * @param   {String} baseDir        - Absolute path of directory to get sidebar
 *     config for.
 * @param   {Object} options        - Options
...
function side(baseDir, {
  ...
  if (currentLevel <= maxLevel) {
    getDirectories(join(baseDir, relativeDir))
        ...
        .forEach(subDir => {
          const children = side(
              baseDir, {stripNumbers, maxLevel, navPrefix, skipEmptySidebar},
              join(relativeDir, subDir), currentLevel + 1);

          if (children.length > 0 || !skipEmptySidebar) {
            // Where to put '02-folder' in ['01-file', { title: 'Other Folder',
            // children: ['03-folder/file'] }]
            const sortedFolderPosition = fileLinks.findIndex(
                link => subDir < (link.children
                                      ? (link.children[0] || "").split(sep)[0]
                                      : link));

            const insertPosition = mixDirectoriesAndFilesAlphabetically &&
                                           sortedFolderPosition > -1
  ...
  if (addReadMeToFirstGroup && fileLinks[0] === "" &&
...
/**
 * Gets sidebar parameters from directory name. Arguments are given after double
 * dash `--` and separated by comma.
 ...
 * @example
 * parseSidebarParameters("docs/api--nc,d2"); { collapsable: false,
 * sidebarDepth: 2 }
 */
 ...
 * @param   {String}          rootDir           - Path of the directory to get
 *     navbar configuration for.
 * @param   {OBject}          options           - Options
 * @param   {String}          relativeDir       - (Used internally for
 *     recursion) Relative directory to `rootDir` to get navconfig for.
 * @param   {Number}          currentNavLevel   - (Used internally for
 *     recursion) Recursion level.
 * @returns {Array.<Object>}
 */
function nav(rootDir, {navPrefix, stripNumbers, skipEmptyNavbar},
             relativeDir = "/", currentNavLevel = 1) {
  relativeDir = relativeDir.replace(/\\/g, "/");
  const baseDir = join(rootDir, relativeDir);
  const childrenDirs =
      getDirectories(baseDir).filter(subDir => subDir.startsWith(navPrefix));
  ...
  if (currentNavLevel > 1 && childrenDirs.length === 0) {
    if (!hasReadme(baseDir)) {
      if (skipEmptyNavbar) {
        ...
        throw new Error(`README.md file cannot be found in ${
            baseDir}. VuePress would return 404 for that NavBar link.`);
      }
    }
    result = {
    ...
    const items =
        childrenDirs
            .map(subDir => nav(rootDir, options, join(relativeDir, subDir),
                               currentNavLevel + 1))
            .filter(Boolean);
    result = currentNavLevel === 1
                 ? items
 ...
 * @param {Object}    nav           - Navigation configuration (Used for
 *     calculating sidebars' roots.)
 * @param {Object}    options       - Options
...
function multiSide(rootDir, nav, options, currentLevel = 1) {
  ...
  nav.forEach(navItem => {
    if (navItem.link) {
      ...
      Object.assign(sideBar, multiSide(rootDir, navItem.items, options),
                    currentLevel + 1);
    }
  });
  ...
  if (currentLevel === 1) {
    ...
    const singleEmptyElement =
        fallBackSide.length === 1 && fallBackSide[0] === ''; // ['']

    if (!options.skipEmptySidebar ||
        (fallBackSide.length > 0 &&
         !singleEmptyElement)) { // If [''] is present at root level, vuepress
                                 // does not render sidebar correctly. See
                                 // `example-with-root-readme` test
...
/**
 * Returns `nav` and `sidebar` configuration for VuePress calculated using
 * structrue of directory and files in given path.
...
function getConfig(rootDirOrOptions, options) {
  let rootDir = typeof rootDirOrOptions === "string"
                    ? normalize(rootDirOrOptions)
                    : join(dirname(callerPath()), '..');
  rootDir = rootDir.endsWith(sep) ? rootDir.slice(0, -1)
                                  : rootDir; // Remove last / if exists.
  ...
  options = {
    ...
    pinyinNav : false,
    ...(options ||
        (typeof rootDirOrOptions === "object" ? rootDirOrOptions : {}))
  };
  ...
  const result = {
    nav : navItems || [],
    sidebar : options.multipleSideBar && navItems
