...
const configOf = (name, options = {}) =>
    getConfig(`${__dirname}/../docs/example-${name}`, options);
...
describe("getConfig", () => {
  ...
  it("should create config without adding readme to first group.", () => {
    expect(configOf("standard", {
      addReadMeToFirstGroup : false
    })).toEqual(expected("standard-no-readme-move"));
  });

  it("should throw for missing README.md in Navbar.", () => {
    expect(() => configOf("missing-readme", {skipEmptyNavbar : false}))
        .toThrow("README.md file cannot be found");
  });
  ...
  it("sort file and folder by prefix.",
     async () => { expect(configOf("sort")).toEqual(expected("sort")); });
